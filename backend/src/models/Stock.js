...
stockSchema.virtual("daysOfCoverage").get(function () {
  ...
  const recentSales = this.salesHistory.filter(
    (h) => h.type === "sale" && h.date > thirtyDaysAgo,
  );

  if (recentSales.length === 0) return null;

  const totalSold = recentSales.reduce(
    (sum, sale) => sum + Math.abs(sale.quantity),
    0,
  );
...
stockSchema.methods.addTransaction = function (
  type,
  quantity,
  value,
  reference = "",
