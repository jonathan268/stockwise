...
const seedData = async () => {
  try {
    ...
    if (isReset) {
      console.log(
        ' Flag --reset détecté. Suppression de toutes les données...'
      );
    ...
    if (!existingAdmin) {
      ...
      console.log(
        ' Compte Super Admin créé avec succès (admin@stockwise.app).'
      );
    } else {
      console.log(' Le compte Super Admin existe déjà.');
    }
